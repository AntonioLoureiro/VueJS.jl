@el(slider,"v-slider",value=200,min=0,max=1000,step=1,label="Use Slider",cols=4)
...
@el(btn_add,"v-btn",content="ADD+10",color="indigo",click="slider.value+=10")
@el(btn_add_100,"v-btn",content="ADD+100",color="indigo",click="slider.value+=100")
@el(alert,"v-alert",content="RESET!",type="success",text=true,cols=12,timeout=5000)
@el(btn_toggle_reset,"v-btn",content="Reset",color="indigo",click="slider.value=0;alert.content='RESET with Click!';alert.value=true")
@el(sel,"v-select",items=["https://google.com","https://weather.com"],label="Select WebSite")
@el(btn_open,"v-btn",content="OPEN",color="indigo",click="open(sel.value)")
